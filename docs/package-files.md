...
Command to verify the package is installed correctly. Should exit with code 0 if installed, non-zero
if not.
...
- For Windows PowerShell commands, consider using `$ErrorActionPreference = "Stop"` for similar
  error handling
