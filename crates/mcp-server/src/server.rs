...
A spec that could not be loaded is reported in the summary's invalid_packages, with its kind, reason and location."
    ...
    #[tool(
        ...
        description = "Check deployed dotfiles for drift between repo sources and targets. Returns per-file drift status. A spec that could not be loaded is reported as structured fields — `kind` (\"yaml\", \"io\", \"unreadable\", \"irregular_file\" or \"refused\"), `reason`, and `line`/`column` where the kind has a location. Branch on `kind`; `reason` is prose for display, not for matching."
    )]
