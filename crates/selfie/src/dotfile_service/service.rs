...
fn load_deploy_state<F: FileSystem>(
    ...
    //
    // `crate::yaml::parse` is what keeps the file's text out: serde-saphyr's
    // `Display` interpolates parsed content into several messages, and the
    // duplicate-key one quotes the key, which here is a dotfile source path. The
    // failure still reports a line and column; the reasoning for accepting that is
    // on `ParseFailure`.
